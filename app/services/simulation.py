...
class RealTimeSimulator:
    ...
    def create_persona(self, persona_type: str, user_hash: str, team_hash: str = None):
        ...
        generators = {
            ...
            # "maria_contagion": self._maria_contagion # Missing in snippets, using fallback or omitting for now
        }
    ...
    def generate_realtime_event(self, user_hash: str, current_risk: str) -> Dict:
        ...
        base_late_prob = {"LOW": 0.1, "ELEVATED": 0.4, "CRITICAL": 0.8}.get(current_risk, 0.1)
        ...
        return {
            ...
            "metadata_": {
                ...
                "comment_length": int(self.rng.exponential(100)) if self.rng.random() > 0.7 else 0
            }
        }
    ...
    def _alex_burnout(self, user_hash: str, team_hash: str) -> List[Event]:
        ...
        for day in range(30):
            ...
            for _ in range(self.rng.integers(3, 6)):
                events.append(Event(
                    user_hash=user_hash,
                    timestamp=current.replace(hour=min(hour, 23), minute=self.rng.integers(0, 60)),
                    event_type="commit",
                    metadata_={
                        "after_hours": late,
                        "context_switches": switches,
                        "is_reply": self.rng.random() > 0.3  # For belongingness calc
                    }
                ))
            ...
            for target in team_hashes[i+1:]:
                if self.rng.random() > 0.3:  # 70% connection probability
                    edges.append(GraphEdge(
                        ...
                        last_interaction=datetime.utcnow() - timedelta(days=int(self.rng.exponential(3))),
