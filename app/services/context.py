...
class ContextEnricher:
    ...
    def _check_seasonal_crunch(self, timestamp: datetime) -> Dict:
        ...
        if month in [10, 11] and timestamp.weekday() >= 4:  # Fri/Sat/Sun
            return {"is_crunch": True, "details": "Black Friday/Holiday prep period"}

        # Tax season (Mar-Apr) for fintech
        if month in [3, 4]:
            return {"is_crunch": True, "details": "Tax season peak"}

        # End of quarter (Mar, Jun, Sep, Dec)
        if timestamp.day >= 25 and ((month % 3) == 0):
            return {"is_crunch": True, "details": "End of quarter push"}

        return {"is_crunch": False, "details": None}

    def mark_events_explained(
        self, events: List[Event], user_email: str
    ) -> List[Event]:
        """
        ...
        for event in events:
            ...
            # Only process after-hours events
            if event.metadata_.get("after_hours"):
